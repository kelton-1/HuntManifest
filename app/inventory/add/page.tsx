...
export default function AddInventoryItemPage() {
    ...
    // Auto-generate name for Decoy and Call categories
    const autoGeneratedName = useMemo(() => {
        if (category === "Decoy" && species && decoyType) {
            return `${species} ${decoyType}s`;
        }
        if (category === "Call" && species) {
            return `${species} Call`;
        }
        return name;
    }, [category, species, decoyType, name]);

    // Determine if form is valid based on category
    const isFormValid = useMemo(() => {
        if (!category) return false;
        if (category === "Decoy") return !!species && !!decoyType;
        if (category === "Call") return !!species;
        return !!name;
    }, [category, species, decoyType, name]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const finalName = (category === "Decoy" || category === "Call")
            ? autoGeneratedName
            : name;
    ...
    return (
        <div className="pb-28 animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {category && (
                    <>
                        {/* Item Name with Suggestions - Hidden for Decoy and Call (auto-generated) */}
                        ...
                        {autoGeneratedName && (
                            <div className="p-4 bg-card rounded-xl border border-border shadow-sm">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-semibold">{autoGeneratedName}</p>
                                        <p className="text-xs text-muted-foreground">
                                            Qty: {quantity}
                                            {brand && ` • ${brand}`}
                                        </p>
